...
$('next').onclick = () => { if (!state.started || state.gameOver) { record('ゲーム開始またはマリガンを選んでください'); render(); return; } if (state.triggerPending) { record('シールドトリガーを処理してください'); render(); return; } if (state.side === 'self' && phases[state.phase] === 'エンド' && state.hand.length > 8) { record(`手札が${state.hand.length}枚です。8枚以下になるよう、選んだカードをリムーブエリアへ送ってください`); render(); return; } saveUndo(); state.phase++; if (state.phase === phases.length) { state.phase = 0; if (state.side === 'self') { state.side = 'opponent'; record('疑似相手のドローフェイズを開始'); } else { state.side = 'self'; state.turn++; startDrawPhase(); } } else if (state.side === 'self' && phases[state.phase] === 'エンド') activateBoardAtEnd(); else record(`${phases[state.phase]}フェーズへ`); render(); };
